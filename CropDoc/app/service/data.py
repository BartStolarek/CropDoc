...
class SampleList(list):
    ...
    def _create_data_map(self,
                         samples: List[Dict]) -> Dict[str, Dict[str, int]]:

        crop_map_dict = {}
        state_map_dict = {}
        for sample in samples:
            if sample['crop_label'] not in crop_map_dict:
                crop_map_dict[sample[
                    'crop_label']] = 1  # Initialize to 1 for the first occurrence
            else:
                crop_map_dict[sample['crop_label']] += 1

            if sample['state_label'] not in state_map_dict:
                state_map_dict[sample[
                    'state_label']] = 1  # Initialize to 1 for the first occurrence
            else:
...
class DatasetManager():
    ...
    def __init__(self,
                 root_path: str,
                 transform: dict[str, Compose],
                 reduce_dataset: float = 0):
        self.root_path = root_path
        self.transform = transform
        self.samples = self._get_samples(
            root_path
        )  # list of samples {img_path, split, crop_label, state_label, idx, crop_idx, state_idx}
    ...
    def _add_state_idx_to_samples(self, samples: SampleList) -> SampleList:
        for sample in samples:
            sample['state_idx'] = self.unique_states.index(
                sample['state_label'])
        return samples
    ...
    def __getitem__(self, idx):
        ...
        crop_label = torch.tensor(self.unique_crops.index(
            sample['crop_label']),
                                  dtype=torch.long)
        state_label = torch.tensor(self.unique_states.index(
            sample['state_label']),
                                   dtype=torch.long)
    ...
    def _train_samples(self, reduce_dataset: float = 0) -> List[Dict]:
        train_samples = [
            sample for sample in self.samples if sample['split'] == 'train'
        ]
        if reduce_dataset:
            train_samples = train_samples[:int(
                len(train_samples) * reduce_dataset)]
        return train_samples

    def _test_samples(self, reduce_dataset: float = 0) -> List[Dict]:
        test_samples = [
            sample for sample in self.samples if sample['split'] == 'test'
        ]
        if reduce_dataset:
            test_samples = test_samples[:int(
                len(test_samples) * reduce_dataset)]
        return test_samples
...
class TransformerManager():
    ...
    def _get_transformers(
            self, transforms: dict[str, list[dict]]) -> dict[str, Compose]:
