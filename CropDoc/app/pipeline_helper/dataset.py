        ...
        for root, directories, files in os.walk(self.root):
            for file in files:
                ...
                try:
                    ...
                    logger.error(f"There was an issue with handling the file, make sure you have the right dataset class and dataset root directory set in the config file. {e}")
                    raise Exception(e)
...
class PlantVillageDataset(BaseDataset):
    ...
    def __init__(self, **kwargs):
        ...
        print(f"self.crops: {self.crops}")
        print(f"self.states: {self.states}")
        print(f"self.crop_index_map: {self.crop_index_map}")
        print(f"self.state_index_map: {self.state_index_map}")
    ...
    def walk_through_root_append_images_and_labels(self):
        ...
        for root, directories, files in os.walk(self.root):
            for directory in directories:
                ...
                logger.debug(f'Found directory with crop: {crop_label} and state: {state_label} and {len(sub_files)} images')
                ...
                for file in sub_files:
                    ...
                    if not file_path.lower().endswith('.jpg') and not file_path.lower().endswith('.jpeg'):
                        logger.debug(f'File {file_path} is not a jpg file, skipping')
                        continue
                    # Append image path and labels
        ...
        logger.info(f'Finished walking through root path, found {len(self.images)} images, data map: {self.data_map}')